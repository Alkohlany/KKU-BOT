...
async def _show_topic_results(query, category_key: str, topic: MenuTopic) -> None:
    ...
    # موضوعات القائمة يتم تنظيمها يدويًا لضمان أن الزر العام لا يعرض منشورًا
    # متشابهًا كتابيًا لكنه مختلف في المعنى. إن حُذف أحد الردود لاحقًا،
    # نستخدم المطابقة المحافظة كخطة احتياطية فقط.
    ...
    text = (
        f"🔎 <b>{topic.label}</b>\n\n"
        "وجدت الموضوعات التالية. اختر المقصود:"
    )
    await _edit_or_reply(
        query,
        text,
