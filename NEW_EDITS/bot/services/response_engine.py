...
# توحيد أكثر الصيغ الجمع/المفرد شيوعاً في أسئلة الطلاب.
TOKEN_EQUIVALENTS = {
    "جداول": "جدول",
    "مواعيد": "موعد",
    "مقررات": "مقرر",
...
def _canonical_token(token: str) -> str:
    ...
    # ضمائر ولواحق بسيطة فقط؛ لا نستخدم جذراً صرفياً واسعاً حتى لا نخلط المعاني.
...
def _score_tokens(query_tokens: tuple[str, ...], pattern_tokens: tuple[str, ...]) -> tuple[float, int, int]:
    ...
    # نحجز كل التطابقات الحرفية أولاً حتى لا تستهلكها مطابقة إملائية أضعف.
    ...
    # لا نقبل الخطأ الإملائي إلا مع وجود تطابق صريح آخر في العبارة.
    if exact:
...
def _score_response(query: str, response: Any) -> ResponseCandidate | None:
    ...
    # تغطية مجموعة صيغ الرد مفيدة عندما يكتب الطالب كلمات موزعة بين أكثر من سطر.
    ...
    if combined_exact >= 2:
        # جمع أسطر الرد يساعد في الاقتراح، لكنه لا يجب أن يصنع إجابة مباشرة وحده.
        ...
        if best is None or (combined_candidate.score, combined_candidate.exact_terms) > (
            ...
            # ترجيح الرد الذي يغطي كلمة توضيحية إضافية مثل «مقرر» أو «فصل».
            best = replace(
                best,
...
def decide_match(query: str, responses: Iterable[Any], max_choices: int = 4) -> MatchDecision:
    ...
    # العبارة المطابقة حرفياً ترسل مباشرة فقط إن كانت فريدة وليست كلمة عامة منفردة.
    if (
        top.exact_phrase
    ...
    # كلمة توضيحية إضافية حسمت التعارض، مثل «اعتذار ... عن مقرر».
    if (
        top.score >= 0.90
    ...
    # تطابق قوي، بعدد كلمات صريحة كافٍ، وبفارق واضح عن المنافس.
    if (
        top.score >= 0.88
    ...
    # عند الغموض نعرض نتائج ذات صلة فقط، ولا نرسل أي منشور قبل اختيار الطالب.
...
def response_label(response: Any, matched_pattern: str | None = None, max_length: int = 48) -> str:
    ...
    # بعض النصوص تحتوي البادئة مرتين؛ نزيلها تكراريًا دون المساس بالعنوان الفعلي.
    for _ in range(2):
