...
_PLAN_REQUEST_RE = re.compile(
    r"^\s*(?:الخطط|الخطه|خطط|خطه)"
    r"(?:\s+(?:الدراسيه|التخصصات|تخصصات|تخصص(?:\s+.+)?|"
    r"دراسيه(?:\s+(?:لتخصص|تخصص)\s+.+)?))?\s*[؟?]?\s*$"
)


def is_study_plan_request(text: str) -> bool:
    """يمنع كلمة «خطة» العامة من اختطاف أسئلة مثل «خطة القبول»."""
...
plans_text_handler = MessageHandler(filters.Regex(r"^\s*(?:الخطط|الخطة|الخطه|خطط|خطة|خطه)(?:\s+(?:الدراسية|الدراسيه|التخصصات|تخصصات|تخصص(?:\s+.+)?|دراسية(?:\s+(?:لتخصص|تخصص)\s+.+)?))?\s*[؟?]?\s*$"), plans_text_command)
