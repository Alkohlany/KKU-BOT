...
def enhance_content(title: str, content: str, image_bytes: bytes = None, mime_type: str = None) -> dict:
    ...
    if has_image:
        prompt = f"""حسّن النص التالي فقط (اللغة، الصياغة، الأخطاء الإملائية). لا تغيّر التنسيق أو الترتيب أو الأسطر أو الإيموجي أو الروابط. لا تضف محتوى جديدًا. أعد النص كما هو مع التحسينات النصية فقط.

أضف وصف الصورة إن وُجدت في النص: حلل الصورة المرفقة واستخرج معلوماتها إن أثرت على المحتوى.
