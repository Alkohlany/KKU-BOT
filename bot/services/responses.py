...
def find_best_match(text, responses):
    ...
    if candidates:
        # ponytail: prefer keyword that appears latest in text (content after greeting)
        candidates.sort(key=lambda r: normalized.find(normalize_arabic(r.keyword.lower().strip())))
        return candidates[-1]
